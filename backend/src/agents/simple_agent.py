...
from backend.src.core.message import Message
...
class SimpleAgent(Agent):
    ...
    def _parse_tool_calls(self, text: str) -> list:
        ...
        支持的格式：
        1. [TOOL_CALL:tool_name:parameters] 格式
        2. JSON格式：[{"action": "tool_name", "action_input": {...}, ...}]
        """
        import json
        ...
        # 格式1.6：兼容模型输出的 "terminal command=..." 或两行 "terminal\ncommand=..."
        if len(tool_calls) == 0 and self.tool_registry:
            available_tools = set(self.tool_registry.list_tools())
            for tool_name in available_tools:
                escaped_tool_name = re.escape(tool_name)
                fallback_patterns = [
                    rf'(?m)^\s*{escaped_tool_name}\s+([^\n]+=[^\n]+)\s*$',
                    rf'(?m)^\s*{escaped_tool_name}\s*\n\s*([^\n]+=[^\n]+)\s*$',
                ]
                for fallback_pattern in fallback_patterns:
                    fallback_match = re.search(fallback_pattern, text)
                    if fallback_match:
                        parameters = fallback_match.group(1).strip().strip('`')
                        tool_calls.append({
                            'tool_name': tool_name,
                            'parameters': parameters,
                            'original': fallback_match.group(0)
                        })
                        break
                if tool_calls:
                    break
        
        # 格式2：解析 JSON 格式（如果没有找到 [TOOL_CALL:] 格式）
        if len(tool_calls) == 0:
    ...
    def run(self, input_text: str, max_tool_iterations: int = 5, **kwargs) -> str:
        """
    ...
    def stream_run(self, input_text: str, max_tool_iterations: int = 5, **kwargs) -> Iterator[str]:
        """
        流式运行Agent，支持工具调用循环
