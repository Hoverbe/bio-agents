...
# ============ 查询分类提示词 ============
# 在进入 master_agent 任务分解前，先用此提示词判断问题复杂度
QUERY_CLASSIFIER_PROMPT = """
你是一个查询分类器，负责判断用户的问题是"简单问题"还是"复杂任务"。

请分析以下用户问题，输出 **SIMPLE** 或 **COMPLEX**。

### 判断标准

**SIMPLE（简单问题）**——直接回答即可，不需要调用工具或多步骤执行：
- 问候、闲聊、寒暄（如"你好"、"今天天气怎么样"）
- 概念解释、知识问答（如"什么是DNA"、"PCR的原理是什么"）
- 任何可以用纯文本直接回答、无需操作外部系统的问题

**COMPLEX（复杂任务）**——需要调用工具、多步骤执行或子Agent协作：
- 需要联网搜索、查询实时信息、获取今天/最新/当前消息
- 需要解析/处理用户上传的文件（PDF、TXT 等）
- 需要查询数据库、调用 MCP 服务或外部 API
- 需要执行脚本、运行命令、处理数据
- 需要多步骤分析流程（如"先查库存再生成报告"）
- 涉及生物信息学分析工具的操作
- 公司制度、SOP 查询
### 用户问题
{user_request}
...
### 输出格式
只输出一个单词：SIMPLE 或 COMPLEX，不要输出任何其他内容。
"""
...
## 思考过程
（用自然语言简要说明你对用户请求的分析，以及为什么这样分解任务）

## 任务列表
（JSON格式的任务列表，每个任务包含 step、agent、task_description、dependency）

示例输出格式：
## 思考过程
用户询问DNA测序原理，这是一个纯知识问答问题，只需调用knowledge_agent即可回答。
