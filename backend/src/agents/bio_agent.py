...
class BioAgent:
    ...
    def _classify_query(self, query: str) -> str:
        """
        判断用户问题是简单问题还是复杂任务，决定是否分发给子Agent。
        ...
        try:
            ...
            logger.warning(f"⚠️ 查询分类失败，默认走复杂任务流程: {e}")
            return "complex"
    ...
    def run(self, query: str, file_context: Optional[str] = None) -> str:
        ...
        try:
            ...
            # 3. 生成最终总结
            final_summary = self._aggregate_results(task_results)
    ...
    def run_research_stream(
        ...
        try:
            ...
            # 发送剩余的缓冲内容（如果还在思考过程部分）
            if not in_task_list_section and buffer.strip():
                yield {
                    'type': 'message_chunk',
                    'agent': 'master_agent',
                    'agent_name': '主控调度专家',
            ...
            if not tasks:
                logger.info("⚠️ 主控 Agent 未返回有效任务列表，直接回答")
                # 流式获取知识 Agent 回答
                yield {
                    'type': 'message',
                    'content': '让我来回答你的问题...',
            ...
            # 4. 主控Agent发送总结报告（流式）
            # 生成详细的总结内容
            summary_content = self._generate_summary(query, tasks, task_results)
